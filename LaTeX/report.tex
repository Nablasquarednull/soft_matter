...
\begin{table}[h]
\centering
\caption{First five values of the end-to-end distance $Q$ and radius of gyration $R_g$ for a freely jointed chain with $N=10$.}
\label{tab:head_data_N10}
\begin{tabular}{cc}
\hline
$Q$ & $R_g$ \\
\hline
...
A considerable ammount of attention has been driven toward the study of polymers that are not large to be on termodynamic equilibrium, the extension of a polymer by the stretching of a cantilever is a recurrent example \cite{Manca} in this regard, an expression for the force - extension relationship has been found by \cite{extensionRef}.  
...
\begin{figure}[ht]
  ...
  \label{subsec:algo1}
  Figure \ref{fig:whatever} shows qualitatively the figures of measure approximated against the theoretical results. It is evident that the scaling of the parameter N does not influence the precision of the figures of measure, as they remain consistent with theory. Moreover, a linear regression shows nearly perfect linear scaling ($\langle Q^2  = t  N^{0.997} \rangle$), which indicates
	\begin{figure}[ht]
		\begin{minipage}{.45\linewidth}
			\centering
...
In conclusion, the numerical simulations performed for the Freely Jointed Chain model successfully reproduce the fundamental statistical and mechanical properties of ideal polymer chains. The mean-square end-to-end distance and the mean-square radius of gyration exhibit the expected linear scaling with the chain length. A log--log linear regression of the simulation data yields $\langle Q^{2} \rangle \sim N^{0.997}$ and $\langle R_{g}^{2} \rangle \sim N^{0.982}$, in excellent agreement with the theoretical prediction of unit scaling exponent for an ideal chain. The probability distribution of the end-to-end distance follows the Gaussian form characteristic of equilibrium FJC statistics, and the computed structure factor is consistent with the Guinier approximation in the low-$k$ regime. Additionally, Monte Carlo simulations of polymer stretching under an external force produce a force--extension relation that closely matches the analytical FJC model, with deviations limited to a few percent of the polymer contour length. These results validate both the numerical methodology and the physical assumptions of the Freely Jointed Chain model, while highlighting the influence of finite-size effects and thermal fluctuations.
